...
def test_widen_when_armed_but_no_quote_refuses_gracefully(server_and_mt5):
    """Gate armed + widening + quote outage: refuse with SYMBOL_NOT_ENABLED
    rather than crashing in the approval-preview build (no live tick to embed).
    Copilot PR #17 finding; mirrors close_position's guard."""
    server, fake = server_and_mt5
...
def test_widen_with_no_quote_auto_executes_when_gate_off(frozen_utc, tmp_path: Path):
    """Gate off (default) + quote outage: widening still auto-executes using the
    position's last-known price for widening detection - no preview, no crash."""
    fake = FakeMT5()
    fake._terminal_info = FakeTerminalInfo(
        time=int(datetime(2026, 4, 21, 13, 0, tzinfo=timezone.utc).timestamp())
    )
